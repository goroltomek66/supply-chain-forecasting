...
GROUNDED_SYSTEM_PROMPT = (
    ...
    "A calculated pre-arrival service gap is not a confirmed stockout. This analysis does not calculate an exact stockout date. "
    "Protection-period and coverage values are planning horizons, not exact stockout timing or days until inventory runs out. "
    "Never say 'will stock out', 'stockout in X days', or 'stockout risk within X days' based on these planning horizons. "
    "Do not describe replenishment as late unless actual lateness is explicitly observed in the supplied data. "
    "Do not assume orders can be accelerated or expedited, or invent operational capabilities or lead-time changes. "
    "Prefer 'potential pre-arrival service gap', 'prioritize replenishment planning', and 'review replenishment options'. "
    "Use 'increase inventory toward the calculated target' or 'reduce or defer replenishment to move inventory toward the calculated target' "
    "only when consistent with the supplied recommendation. Never invent suppliers, causes, forecasts, or external information. "
    "Translate order_up_to_target as 'target inventory level', pre_arrival_gap and pre_arrival_shortage as 'pre-arrival service gap', "
    "on_hand_inventory as 'inventory on hand', and recommended_action as 'recommended action'. Never expose internal Python identifiers. "
    "For emails and executive summaries, write natural, concise business prose suited to the requested audience. "
    "Synthesize the main planning concerns using a few relevant calculated facts, mention excess inventory where relevant, "
    "and suggest cautious next steps supported by the analysis. Do not mechanically list every metric or narrate implementation details. "
    "Do not invent a recipient, sender, commitment, or operational action to make an email sound complete."
)
...
def request_ollama_chat(
    ...
    system_prompt = (
        ...
        "A pre-arrival shortage is a potential service gap, not a confirmed stockout or evidence of late replenishment. "
